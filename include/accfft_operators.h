...
template <typename T, typename Tp> void accfft_grad_t      (T* A_x , T* A_y, T* A_z, T* A, Tp *plan, std::bitset<3>* pXYZ=NULL, double* timer=NULL);
...
template <typename T, typename Tp> void accfft_biharmonic_t   (T* LA  , T* A  , Tp* plan, double* timer=NULL);

void accfft_grad      (double* A_x , double* A_y, double* A_z, double* A, accfft_plan *plan, std::bitset<3>* pXYZ=NULL, double* timer=NULL);
...
void accfft_biharmonicf   (float* LA  , float* A  , accfft_planf* plan, double* timer=NULL);
#endif
