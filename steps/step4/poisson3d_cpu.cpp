 ...
 *
 * Example of use:
 * ./poisson3d_cpu -x 64 -y 64 -z 64 -m 1 -t 2
 *
...
void poisson_solve(PoissonParams &params, int nthreads) {
  ...
  switch(testCaseNb) {
  case TESTCASE_SINE:
    initialize<TESTCASE_SINE>(data, exact_solution, n, c_comm, params); 
    break;
  case TESTCASE_GAUSSIAN:
    initialize<TESTCASE_GAUSSIAN>(data, exact_solution, n, c_comm, params); 
    break;
  case TESTCASE_UNIFORM_BALL:
    initialize<TESTCASE_UNIFORM_BALL>(data, exact_solution, n, c_comm, params); 
    break;
  }
  MPI_Barrier(c_comm);

  // optional : save rho (rhs) data
#ifdef USE_PNETCDF
  {
  ...
  poisson_fourier_filter(data_hat, n, osize, ostart, methodNb);
  ...
  rescale_numerical_solution(data2, isize, params);

  /* L2 error between phi and phi_exact */
  compute_L2_error(data2, exact_solution, isize, params);
