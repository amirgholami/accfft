...
inline double testcase(double X,double Y,double Z){
  ...
  double pi=M_PI;
  double analytic;
  analytic= C0*sin(X)*sin(Y)*sin(Z)+C1*std::exp( -SIGMA * ( (X-pi)*(X-pi) + (Y-pi)*(Y-pi) + (Z-pi)*(Z-pi) ));
...
inline double solution_grad(double X,double Y,double Z,int direction){
  ...
  double analytic=0;
  if(direction==0)
   analytic= C0*cos(X)*sin(Y)*sin(Z)+C1*(-2*SIGMA*(X-pi))*std::exp( -SIGMA * ( (X-pi)*(X-pi) + (Y-pi)*(Y-pi) + (Z-pi)*(Z-pi) ));
  else if(direction==1)
   analytic= C0*sin(X)*cos(Y)*sin(Z)+C1*(-2*SIGMA*(Y-pi))*std::exp( -SIGMA * ( (X-pi)*(X-pi) + (Y-pi)*(Y-pi) + (Z-pi)*(Z-pi) ));
  else if(direction==2)
   analytic= C0*sin(X)*sin(Y)*cos(Z)+C1*(-2*SIGMA*(Z-pi))*std::exp( -SIGMA * ( (X-pi)*(X-pi) + (Y-pi)*(Y-pi) + (Z-pi)*(Z-pi) ));
...
inline double solution_laplace(double X,double Y,double Z){
  ...
  double pi=M_PI;
  double analytic=0;
  analytic+= C1*(4*SIGMA*SIGMA*(X-pi)*(X-pi))*std::exp( -SIGMA * ( (X-pi)*(X-pi) + (Y-pi)*(Y-pi) + (Z-pi)*(Z-pi) ));
  analytic+= C1*(4*SIGMA*SIGMA*(Y-pi)*(Y-pi))*std::exp( -SIGMA * ( (X-pi)*(X-pi) + (Y-pi)*(Y-pi) + (Z-pi)*(Z-pi) ));
  analytic+= C1*(4*SIGMA*SIGMA*(Z-pi)*(Z-pi))*std::exp( -SIGMA * ( (X-pi)*(X-pi) + (Y-pi)*(Y-pi) + (Z-pi)*(Z-pi) ));
  analytic+= C1*(-6*SIGMA)*std::exp( -SIGMA * ( (X-pi)*(X-pi) + (Y-pi)*(Y-pi) + (Z-pi)*(Z-pi) ));
...
inline double solution_biharmonic(double X,double Y,double Z){
  ...
  double f=std::exp( -SIGMA * ( (X-pi)*(X-pi) + (Y-pi)*(Y-pi) + (Z-pi)*(Z-pi) ));
  double Xp=X-pi;
  double Yp=Y-pi;
  double Zp=Z-pi;
  ...
  double fxxxx=12*SIGMA2*f-48*SIGMA3*Xp*Xp*f+16*SIGMA4*Xp*Xp*Xp*Xp*f;
  double fyyyy=12*SIGMA2*f-48*SIGMA3*Yp*Yp*f+16*SIGMA4*Yp*Yp*Yp*Yp*f;
  double fzzzz=12*SIGMA2*f-48*SIGMA3*Zp*Zp*f+16*SIGMA4*Zp*Zp*Zp*Zp*f;
  double fxxyy=4*SIGMA2*f-8*SIGMA3*Yp*Yp*f-8*SIGMA3*Xp*Xp*f+16*SIGMA4*Xp*Xp*Yp*Yp*f;
  double fxxzz=4*SIGMA2*f-8*SIGMA3*Zp*Zp*f-8*SIGMA3*Xp*Xp*f+16*SIGMA4*Xp*Xp*Zp*Zp*f;
  double fyyzz=4*SIGMA2*f-8*SIGMA3*Zp*Zp*f-8*SIGMA3*Yp*Yp*f+16*SIGMA4*Yp*Yp*Zp*Zp*f;
...
void biharmonic(int *n, int nthreads) {
  ...
  double * biharmonic=(double*)accfft_alloc(isize[0]*isize[1]*isize[2]*sizeof(double));
  double timings[5]={0};
  ...
  PCOUT<<"Timing for Biharmonic Computation for size "<<n[0]<<"*"<<n[1]<<"*"<<n[2]<<std::endl;
  PCOUT<<"Setup \t\t"<<g_setup_time<<std::endl;
  PCOUT<<"Evaluation \t"<<g_exec_time<<std::endl;
  ...
  for (int i=0; i<isize[0]; i++){
    for (int j=0; j<isize[1]; j++){
      for (int k=0; k<isize[2]; k++){
        ...
        err+=std::abs(numerical_r-solution_grad(X,Y,Z,direction));
