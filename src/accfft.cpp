...
int accfft_local_size_dft_r2c(int * n, int * isize, int * istart, int * osize,
	...
	n_tuples = (n[2] / 2 + 1) * 2;
	alloc_local = dfft_get_local_size(n[0], n[1], n_tuples, osize_0, ostart_0,
			c_comm);
	alloc_max = std::max(alloc_max, alloc_local);
	alloc_local = dfft_get_local_size(n[0], n_tuples / 2, n[1], osize_1,
			ostart_1, c_comm);
	alloc_max = std::max(alloc_max, alloc_local * 2);
	alloc_local = dfft_get_local_size(n[1], n_tuples / 2, n[0], osize_2,
			ostart_2, c_comm);
	alloc_max = std::max(alloc_max, alloc_local * 2);
...
accfft_plan* accfft_plan_dft_3d_r2c(int * n, double * data, double * data_out,
  ...
  int* isize = plan->isize;
	...
	alloc_max = accfft_local_size_dft_r2c(n, plan->isize, plan->istart,
...
int accfft_local_size_dft_c2c(int * n, int * isize, int * istart, int * osize,
	...
	alloc_local = dfft_get_local_size(n[0], n[1], n[2], osize_0, ostart_0,
			c_comm);
	alloc_max = std::max(alloc_max, alloc_local);
	alloc_local = dfft_get_local_size(n[0], n[2], n[1], osize_1, ostart_1,
			c_comm);
	alloc_max = std::max(alloc_max, alloc_local);
	alloc_local = dfft_get_local_size(n[1], n[2], n[0], osize_2, ostart_2,
			c_comm);
	alloc_max = std::max(alloc_max, alloc_local);
