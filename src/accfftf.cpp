...
int dfft_get_local_sizef(int N0, int N1, int N2, int * isize, int * istart,MPI_Comm c_comm ){
  int nprocs, procid;
...
int accfft_local_size_dft_r2cf( int * n,int * isize, int * istart, int * osize, int *ostart,MPI_Comm c_comm){
  ...
  n_tuples=(n[2]/2+1)*2;
  alloc_local=dfft_get_local_sizef(n[0],n[1],n_tuples,osize_0,ostart_0,c_comm);
  alloc_max=std::max(alloc_max, alloc_local);
  alloc_local=dfft_get_local_sizef(n[0],n_tuples/2,n[1],osize_1,ostart_1,c_comm);
  alloc_max=std::max(alloc_max, alloc_local*2);
  alloc_local=dfft_get_local_sizef(n[1],n_tuples/2,n[0],osize_2,ostart_2,c_comm);
  alloc_max=std::max(alloc_max, alloc_local*2);
...
accfft_planf*  accfft_plan_dft_3d_r2cf(int * n, float * data, float * data_out, MPI_Comm c_comm,unsigned flags){
  ...
  // FFT Plans
  {
    plan->fplan_0= fftwf_plan_many_dft_r2c(1, &n[2],osize_0[0]*osize_0[1], //int rank, const int *n, int howmany
    ...
    plan->iplan_0= fftwf_plan_many_dft_c2r(1, &n[2],osize_0[0]*osize_0[1], //int rank, const int *n, int howmany
    ...
    plan->fplan_1=fftwf_plan_guru_dft(
        1, &dims,
        2,howmany_dims,
        (fftwf_complex*)data_out, (fftwf_complex*)data_out,
        -1,fftw_flags);
    if(plan->fplan_1==NULL) std::cout<<"!!! fplan1 not created in r2c plan!!!"<<std::endl;
    plan->iplan_1=fftwf_plan_guru_dft(
        1, &dims,
        2,howmany_dims,
        (fftwf_complex*)data_out, (fftwf_complex*)data_out,
        1,fftw_flags);
  ...
  if (plan->oneD){
    ...
    plan->T_plan_2  = new T_Plan<float>(n[0],n[1],n_tuples_o, plan->Mem_mgr, c_comm);
    plan->T_plan_2i = new T_Plan<float>(n[1],n[0],n_tuples_o,plan->Mem_mgr, c_comm);
    plan->T_plan_1  =NULL;
    plan->T_plan_1i =NULL;
  ...
  // 2D Decomposition
  if (!plan->oneD){
    plan->Mem_mgr   = new Mem_Mgr<float>(n[1],n_tuples_o/2,2,plan->row_comm,osize_0[0],alloc_max);
    plan->T_plan_1  = new T_Plan<float>(n[1],n_tuples_o/2,2, plan->Mem_mgr, plan->row_comm,osize_0[0]);
    plan->T_plan_2  = new T_Plan<float>(n[0],n[1],osize_2[2]*2, plan->Mem_mgr, plan->col_comm);
    plan->T_plan_2i = new T_Plan<float>(n[1],n[0],osize_2i[2]*2, plan->Mem_mgr, plan->col_comm);
    plan->T_plan_1i = new T_Plan<float>(n_tuples_o/2,n[1],2, plan->Mem_mgr, plan->row_comm,osize_1i[0]);
...
void accfft_executef(accfft_planf* plan, int direction,float * data, float* data_out, double * timer,std::bitset<3> XYZ){
  ...
  if(direction==-1){
    ...
    fft_time-=MPI_Wtime();
    if(XYZ[1])
      fftwf_execute_dft(plan->fplan_1,(fftwf_complex*)data_out,(fftwf_complex*)data_out);
    fft_time+=MPI_Wtime();
    ...
    fft_time-=MPI_Wtime();
    if(XYZ[1])
      fftwf_execute_dft(plan->iplan_1,(fftwf_complex*)data,(fftwf_complex*)data);
    fft_time+=MPI_Wtime();
...
int accfft_local_size_dft_c2cf( int * n,int * isize, int * istart, int * osize, int *ostart,MPI_Comm c_comm){
  ...
  alloc_local=dfft_get_local_sizef(n[0],n[1],n[2],osize_0,ostart_0,c_comm);
  alloc_max=std::max(alloc_max, alloc_local);
  alloc_local=dfft_get_local_sizef(n[0],n[2],n[1],osize_1,ostart_1,c_comm);
  alloc_max=std::max(alloc_max, alloc_local);
  alloc_local=dfft_get_local_sizef(n[1],n[2],n[0],osize_2,ostart_2,c_comm);
  alloc_max=std::max(alloc_max, alloc_local);
...
accfft_planf*  accfft_plan_dft_3d_c2cf(int * n, Complexf * data, Complexf * data_out, MPI_Comm c_comm,unsigned flags){
  ...
  if (plan->oneD){
    ...
    plan->T_plan_2  = new T_Plan<float>(NX,NY,(NZ)*2, plan->Mem_mgr,c_comm);
    plan->T_plan_2i = new T_Plan<float>(NY,NX,NZ*2, plan->Mem_mgr,c_comm);
  ...
  if (!plan->oneD){
    ...
    plan->Mem_mgr   =new Mem_Mgr<float>(n[1],n[2],2,plan->row_comm,osize_0[0],alloc_max);
    plan->T_plan_1  =new T_Plan<float>(n[1],n[2],2, plan->Mem_mgr, plan->row_comm,osize_0[0]);
    plan->T_plan_2  =new T_Plan<float>(n[0],n[1],2*osize_2[2], plan->Mem_mgr, plan->col_comm);
    plan->T_plan_2i =new T_Plan<float>(n[1],n[0],2*osize_2i[2], plan->Mem_mgr, plan->col_comm);
    plan->T_plan_1i =new T_Plan<float>(n[2],n[1],2, plan->Mem_mgr, plan->row_comm,osize_1i[0]);
    ...
    if(flags==ACCFFT_MEASURE){
      if(coord[0]==0){
        plan->T_plan_1->which_fast_method(plan->T_plan_1,(float*)data_out,osize_0[0]);
      }
    }
    else{
      if(coord[0]==0){
...
void accfft_execute_c2cf(accfft_planf* plan, int direction,Complexf * data, Complexf * data_out, double * timer,std::bitset<3> XYZ){
  ...
  if(direction==-1){
    ...
    if(!plan->oneD){
      plan->T_plan_1->execute(plan->T_plan_1,(float*)data_out,timings,2,osize_0[0],coords[0]);
    }
    /**************************************************************/
    /*******************  N0/P0 x N1 x N2/P1 **********************/
    /**************************************************************/
    ...
    if(!plan->oneD){
      plan->T_plan_2->execute(plan->T_plan_2,(float*)data_out,timings,2,1,coords[1]);
    }
    else{
      plan->T_plan_2->execute(plan->T_plan_2,(float*)data_out,timings,2);
    }
    /**************************************************************/
    /*******************  N0 x N1/P0 x N2/P1 **********************/
    /**************************************************************/
    ...
    if(!plan->oneD){
      plan->T_plan_2i->execute(plan->T_plan_2i,(float*)data,timings,1,1,coords[1]);
    }
    else{
    ...
    if(!plan->oneD){
      plan->T_plan_1i->execute(plan->T_plan_1i,(float*)data,timings,1,osize_1i[0],coords[0]);
    }
    MPI_Barrier(plan->c_comm);
    /**************************************************************/
    /*******************  N0/P0 x N1/P1 x N2 **********************/
...
void accfft_destroy_plan(accfft_planf * plan){
  ...
  if(plan!=NULL){
    ...
    if(plan->fplan_0!=NULL) fftwf_destroy_plan(plan->fplan_0);
    if(plan->fplan_1!=NULL) fftwf_destroy_plan(plan->fplan_1);
    if(plan->fplan_2!=NULL) fftwf_destroy_plan(plan->fplan_2);
    if(plan->iplan_0!=NULL) fftwf_destroy_plan(plan->iplan_0);
    if(plan->iplan_1!=NULL) fftwf_destroy_plan(plan->iplan_1);
    if(plan->iplan_2!=NULL) fftwf_destroy_plan(plan->iplan_2);
...
void accfft_execute_r2c_t(accfft_planf* plan, T* data, Tc* data_out, double* timer,std::bitset<3> XYZ){
  accfft_execute_r2cf(plan,data,data_out,timer,XYZ);
  return;
}
template <typename Tc, typename T>
void accfft_execute_c2r_t(accfft_planf* plan, Tc* data,T* data_out, double * timer,std::bitset<3> XYZ){
  accfft_execute_c2rf(plan,data,data_out,timer,XYZ);
  return;
}

template void accfft_execute_r2c_t<float,Complexf>(accfft_planf* plan, float* data,Complexf* data_out, double * timer,std::bitset<3> XYZ);
template void accfft_execute_c2r_t<Complexf,float>(accfft_planf* plan, Complexf* data,float* data_out, double * timer,std::bitset<3> XYZ);
...
template int accfft_local_size_dft_r2c_t<float>( int * n,int * isize, int * istart, int * osize, int *ostart,MPI_Comm c_comm);
template int accfft_local_size_dft_r2c_t<Complexf>( int * n,int * isize, int * istart, int * osize, int *ostart,MPI_Comm c_comm);
